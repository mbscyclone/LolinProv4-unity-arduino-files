...
void httpheader(WiFiClient xilent) {
  ...
  xilent.println("<p><form action=\"/pinayar\" method=\"POST\"><input type=\"submit\" value=\"pin ayarlari\"></form></p></td>");
  xilent.println("<td style=\"border:1px solid black;width:250px;\">");
...
if (Menu == 1) {
            ...
            if (header.indexOf("/programkayit?is=") > -1) {
              ...
              //dosyaOkupinayar();
              httpheader(xilent);
              xilent.println("<br>Giris formati (Bosluk kullanma, Bosluklarin yerine \"|\" isareti kullanilacak.)<br>pinAdi pinModu pinSinyalTipi pinMinDegeri pinDegeri pinMaxDegeri pinEtiketi");
              xilent.println("<form method='get' action='pinkayit'><br><label>pin ayarlari:</label><br>");
              xilent.println("<textarea name='is' id='id' cols='50' rows='10' >");
              xilent.println(pinayar);
              xilent.println("</textarea>");
              xilent.println("<br>Kaydedince Restart olabilir. ESP ye bağlı Cihazları kapatın.<br>");
              ...
              xilent.println("D2|OUT|DIG|0|0|0|Temp_Hum<br>");
              xilent.println("D3|OUT|DIG|0|0|1|DC_Motor<br>");
              xilent.println("A0|INP|ANG|0|0|1024|Var_Resistor<br>");
              xilent.println("<br><br>");
            ...
            if (header.indexOf(" /pinayar?is=") > -1) {
              ...
              httpheader(xilent);
              xilent.println("<br>Giris formati (Bosluk kullanma, Bosluklarin yerine \"|\" isareti kullanilacak.)<br>pinAdi pinModu pinSinyalTipi pinMinDegeri pinDegeri pinMaxDegeri pinEtiketi");
              xilent.println("<form method='get' action='pinayar'><br><label>pin ayarlari:</label><br>");
              xilent.println("<textarea name='is' id='id' cols='50' rows='10' >");
              xilent.println(pinayar);
              xilent.println("</textarea>");
              xilent.println("<br>Kaydedince Restart olabilir. ESP ye bağlı Cihazları kapatın.<br>");
            ...
            if (header.indexOf("/ssidset?") > -1) {  //   "/ssidset?ssid=Zyxel&pass=bebedede14"
              ...
              xilent.println(ipStr);
              xilent.println("<br>");
              xilent.println("<form action=\"/ssidset\" method=\"get\"><label>SSID: </label><input name='ssid' length=32><label>PASS: </label><input name='pass' length=32><input type='submit'></form>");
              xilent.println("<br>");
              xilent.println("<br><br>");
...
if(header.indexOf("?fbeml=")>-1)

{
           String hdrt=header;
           header = Karakterduzeltfunc(hdrt);
//Serial.println(header);
  USER_EMAIL=header.substring(header.indexOf("?fbeml=")+7,header.indexOf("&fbupw="));
...
if(fben>0 && DATABASE_URL !="" && API_KEY != "" && USER_EMAIL != "" && USER_PASSWORD != "")
{
  errstring ="O";
}
...
// autocomplete=\"off\"
              ...
              xilent.println("<label>FBRTD nin URL yi buraya yazın <br> URL  : </label>");
              xilent.println("<input name='fburl' id='fburl' style=\"width:350px;\" value='");
              xilent.println(DATABASE_URL);
              xilent.println("'><br><br>");
              xilent.println("<label>Firebase Realtime Database API_KEY <br> buraya yazın : <br> API_KEY : </label><input name='fbapi' id='fbapi' style=\"width:320px;\" value='");
              xilent.println(API_KEY);
              xilent.println("'><br><br>");
              xilent.println("<label>Database deki Yol (Örnek: Balıkesir ev1: bev1, yazlık 10Yz1, gibi kısa kodlar kullanın) <br> Yol kodunu buraya yazın : <br> YOL : </label><input name='fbyol' id='fbyol' style=\"width:70px;\" value='");
              xilent.println(YOL);
              xilent.println("'>    <input type='submit'>");
              xilent.println("</form>");
              xilent.println("<hr style=\"height:4px;border-width:0;color:black;background-color:gray\">");
              ...
              xilent.println("<form method='get' action='/firebaseset'>");
              xilent.println("<label>Auth kullanıcı e-mailini buraya yazın <br> E-MAIL : </label>");
              xilent.println("<input name='fbeml' id='fbeml' style=\"width:150px;\" value='");
              xilent.println(USER_EMAIL);
              xilent.println("'>@gmail.com<br><br>");
              xilent.println("<label>Auth kullanıcı şifresini buraya yazın : <br> PASSWORD : </label><input name='fbupw' id='fbupw' style=\"width:120px;\" value='");
              ...
              if(errstring=="O")
              {
              xilent.println("<form method='get' action='/esprestart'>");
              xilent.println("<br><label>Eğer ayarlar doğru Firebase protokolü başlatmak için ise.. <br><br> Restart gerekli</label>");
              xilent.println("<input type='submit' value='Restart'>");
              xilent.println("</form>");
              errstring="";
              }
              ...
              xilent.println("<br><br>"+errstring+"<br><br><br><br></body>");
              xilent.println("</html>");
            ...
            // Clear the header variable
            xilent.println("<br><br><br><br><br>coded by " + creator + ". ESP control  (ver:01__01.09.2025)<br><br><br>");
            xilent.println();
            /////xilent.abort();;
}
...
*/
           ...
           // xilent.println("<form action=\"/rr0\" method=\"POST\"><input type=\"submit\" value=\"Kapat\" style='width:50px;'></form></td>");
            ...
            xilent.println("<br>");
            if(fben==0)xilent.println("Firebase'i kullanıcı açmamış)");
            if(fben==1)xilent.println("Firebase Açık");
            if(fben>7)xilent.println("Firebase Kapalı (Hata oldu) test: "+ String(fben-1));
            
            if(fberror != "" || fben!=1){xilent.println("<br> FB hatası: " + fberror);
            xilent.println("<form action=\"/firebaseset?fireb=On\" method=\"POST\"><input type=\"submit\" value=\"FB Ac\"></form>");
            }
            
            if(Firebase.ready())xilent.println("<form action=\"/firebaseset?fireb=Off\" method=\"POST\"><input type=\"submit\" value=\"FB Kapat\"></form>");
