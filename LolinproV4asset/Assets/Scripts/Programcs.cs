...
public class Programcs : MonoBehaviour
{
    ...
    public void rguncelle() {
        ...
        for (int i=1;i<4;i++)
        {
            ...
            if ((rtmp.IndexOf("\",")>-1)) rtmp = rtmp.Substring(rtmp.IndexOf("\"")+1, rtmp.Length - (rtmp.IndexOf("\",") + 1)); // ",
            else 
            {
                rtmp = "";
                break;
            }
            // aradaki sayýyý silelim
            rtmp = rtmp.Substring(rtmp.IndexOf(":\"") + 2, rtmp.Length - (rtmp.IndexOf(":\"") + 2)); // :" bundan sonra sayý al
            rtmp = rtmp.Substring(rtmp.IndexOf(",") + 1, rtmp.Length - (rtmp.IndexOf(",") + 1));    // , sayýyý geçtik
